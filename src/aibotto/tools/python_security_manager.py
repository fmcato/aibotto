...
class PythonSecurityManager:
    ...
    async def validate_python_code(self, code: str) -> dict[str, object]:
        ...
        logger.debug(f"PYTHON SECURITY CHECK: Starting validation for code (length: {len(code)}, limit: {self.max_python_code_length})")
        ...
        if length_check:
            logger.warning(f"PYTHON SECURITY CHECK: Blocked by length check - {length_check['message']}")
            return length_check

        # Check for blocked patterns (exec, subprocess, etc.)
        pattern_check = await self._check_blocked_patterns(code)
        if pattern_check:
            logger.warning(f"PYTHON SECURITY CHECK: Blocked by pattern check - {pattern_check['message']}")
            return pattern_check

        # Check import restrictions
        import_check = await self._check_import_restrictions(code)
        if import_check:
            logger.warning(f"PYTHON SECURITY CHECK: Blocked by import check - {import_check['message']}")
            return import_check

        # Check custom blocked patterns
        custom_pattern_check = await self._check_custom_patterns(code)
        if custom_pattern_check:
            logger.warning(f"PYTHON SECURITY CHECK: Blocked by custom pattern check - {custom_pattern_check['message']}")
    ...
    async def _check_python_code_length(self, code: str) -> dict[str, object] | None:
        ...
        logger.debug(f"PYTHON LENGTH CHECK: Code length={len(code)}, max={self.max_python_code_length}")
        if len(code) > self.max_python_code_length:
            message = (
                f"Error: Python code too long (max {self.max_python_code_length} characters)"
            )
            if self.enable_audit_logging:
    ...
    async def _check_blocked_patterns(self, code: str) -> dict[str, object] | None:
        ...
        logger.debug(f"PYTHON BLOCKED PATTERNS CHECK: Checking {len(self.blocked_patterns)} blocked patterns")

        for pattern in self.blocked_patterns:
            if pattern in code_lower:
                logger.warning(f"PYTHON BLOCKED PATTERNS CHECK: MATCHED - found '{pattern}' in code")
    ...
    async def _check_import_restrictions(self, code: str) -> dict[str, object] | None:
        ...
        for import_stmt in import_statements:
            ...
            if not is_allowed:
                logger.warning(f"PYTHON IMPORT CHECK: BLOCKED - import '{import_stmt}' not in allowed list")
                return self._create_blocked_result(
    ...
    async def _check_custom_patterns(self, code: str) -> dict[str, object] | None:
        ...
        for pattern in self.custom_blocked_patterns:
            if pattern and pattern in code_lower:
                message = "Error: Python code matches blocked pattern for security reasons"
                if self.enable_audit_logging:
