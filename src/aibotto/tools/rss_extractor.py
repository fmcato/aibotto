...
class RSSExtractor:
    ...
    def is_rss_feed(self, content: str, content_type: str = "") -> bool:
        ...
        if content_type:
            ...
            if any(
                ...
                for ct in ["application/rss+xml", "text/xml", "application/xml"]
            ):
    ...
    def _extract_rss_2_0(self, root: Any, url: str) -> dict[str, Any]:
        """Extract content from RSS 2.0 feed."""
        channel = root.find("channel")
        title = (
            channel.findtext("title", "").strip() if channel is not None else "RSS Feed"
        )
        description = (
            channel.findtext("description", "").strip() if channel is not None else ""
        )
    ...
    def _extract_atom(self, root: Any, url: str) -> dict[str, Any]:
        ...
        title = (
            root.findtext("title", "") or root.findtext("atom:title", "", ns)
        ).strip() or "Atom Feed"
        subtitle = (
            root.findtext("subtitle", "") or root.findtext("atom:subtitle", "", ns)
        ).strip() or ""
        ...
        entry_list = (
            root.findall(".//entry") if not ns else root.findall(".//atom:entry", ns)
        )
        for entry in entry_list:
            ...
            entry_title = (
                entry.findtext("title", "") or entry.findtext("atom:title", "", ns)
            ).strip()

            # Link element in Atom is an attribute
            link_elem = entry.find("link") if not ns else entry.find("atom:link", ns)
            entry_link = (
                link_elem.get("href", "").strip() if link_elem is not None else ""
            )

            entry_summary = (
                entry.findtext("summary", "") or entry.findtext("atom:summary", "", ns)
            ).strip()
            entry_content = (
                entry.findtext("content", "") or entry.findtext("atom:content", "", ns)
            ).strip()
            entry_updated = (
                entry.findtext("updated", "") or entry.findtext("atom:updated", "", ns)
            ).strip()
        ...
        content = (
            f"Feed Description: {subtitle or title}\n\nLatest Entries:\n"
            + "\n\n".join(entries)
        )
    ...
    def _extract_rss_1_0(self, root: Any, url: str) -> dict[str, Any]:
        """Extract content from RSS 1.0 feed."""
        # RSS 1.0 uses RDF
        channel = root.find(".//channel")
        title = (
            channel.findtext("title", "").strip()
