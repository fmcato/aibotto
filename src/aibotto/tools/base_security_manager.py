...
class BaseSecurityManager(ABC):
    ...
    def _initialize_properties(self, max_length: int | None = None) -> None:
        ...
        self.max_length = max_length if max_length is not None else self._get_max_length()
    ...
    async def validate_input(self, input_data: str) -> dict[str, Any]:
        ...
        logger.debug(f"SECURITY CHECK: Starting validation for input (length: {len(input_data)}, limit: {self.max_length})")
        ...
        for step in validation_steps:
            ...
            if check_result:
                logger.warning(f"SECURITY CHECK: Blocked by {step.__name__} - {check_result['message']}")
                return check_result
    ...
    async def _check_blocked_items(self, input_data: str) -> dict[str, Any] | None:
        ...
        for item in self.blocked_items:
            if item in input_data:
                return self._create_blocked_result_dict(f"Blocked item detected: {item}")
        return None
    ...
    async def _check_custom_patterns(self, input_data: str) -> dict[str, Any] | None:
        ...
        for pattern in self.custom_blocked_patterns:
            if pattern in input_data:
                return self._create_blocked_result_dict(f"Custom blocked pattern detected: {pattern}")
        return None
