    ...
    def reload_from_file(cls, config_file: str = "security_config.json") -> None:
        ...
        try:
            ...
            if os.path.exists(config_file):
                ...
                cls.MAX_COMMAND_LENGTH = config.get("MAX_COMMAND_LENGTH", cls.MAX_COMMAND_LENGTH)
                cls.ALLOWED_COMMANDS = config.get("ALLOWED_COMMANDS", cls.ALLOWED_COMMANDS)
                cls.BLOCKED_COMMANDS = config.get("BLOCKED_COMMANDS", cls.BLOCKED_COMMANDS)
                cls.CUSTOM_BLOCKED_PATTERNS = config.get("CUSTOM_BLOCKED_PATTERNS", cls.CUSTOM_BLOCKED_PATTERNS)
                cls.ENABLE_AUDIT_LOGGING = config.get("ENABLE_AUDIT_LOGGING", cls.ENABLE_AUDIT_LOGGING)
