#!/usr/bin/env python3
"""
Test to reproduce the exact Python security issue with the actual Python executor.
"""
...
class MockPythonExecutor:
    """Mock Python executor that simulates the real one."""
    
    def __init__(self):
        self.security_manager = MockSecurityManager(max_length=1000)
    
    async def execute_python_code(self, code: str) -> str:
        """Execute Python code with security validation."""
        # This simulates what the real Python executor does
        full_command = f"python3 -c {repr(code)}"
        
        print(f"PYTHON EXECUTOR: Validating command: {full_command}")
        
        # Validate with security manager
        security_result = await self.security_manager.validate_command(full_command)
        
        if not security_result['allowed']:
            return f"Python code blocked for security: {security_result['message']}"
...
async def test_python_executor():
    """Test Python executor that should be blocked."""
    
    print("Testing Python executor with security...")
    
    executor = MockPythonExecutor()
    
    # Test cases that would be blocked in real scenario
    test_cases = [
